...
from services.item_service import (
    servicio_crear_item,
    servicio_actualizar_item,
    servicio_actualizar_servicio,
)
...
@item_bp.route("/item/servicio/<id_item>", methods=["PUT", "OPTIONS"])
@item_bp.route("/item/servicio/<id_item>/", methods=["PUT", "OPTIONS"])
def actualizar_servicio(id_item: str):
    if request.method == "OPTIONS":
        return "", 204
    ...
    try:
        res = servicio_actualizar_servicio(body, id_item=id_item, id_empresa=id_empresa, user_id=user_id)
        return jsonify(res), 200
    except ValidationError as ve:
        return jsonify({"success": False, "errors": ve.messages}), 400
    except LookupError:
        return jsonify({"success": False, "error": "Servicio no encontrado o sin permiso para actualizarlo"}), 404
    except IntegrityError as e:
        return jsonify({"success": False, "error": "Violación de integridad en base de datos", "detail": str(e)}), 409
    except SQLAlchemyError as e:
        return jsonify({"success": False, "error": "Error de base de datos", "detail": str(e)}), 500
