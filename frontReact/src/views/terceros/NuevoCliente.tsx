...
const NuevoCliente: React.FC = () => {
  ...
  const onSubmitRHF = useCallback(async (values: NuevoTerceroFormValues) => {
    ...
    try {
      ...
      const errorMessage = e?.response?.data?.error || e?.message || 'Error al crear el cliente';
      setErr(errorMessage);
    } finally {
      setLoading(false);
    }
      ...
      <Card>
        <CardBody>
          <div className="d-flex justify-content-between align-items-center mb-4">
            ...
            <div>
              ...
              <Button color="primary" onClick={handleSubmit(onSubmitRHF, onInvalid)} disabled={loading}>
                {loading ? (<><Spinner size="sm" className="me-2" />Guardando…</>) : 'Crear Cliente'}
              </Button>
            </div>
          </div>

          {ok && <Alert color="success">Cliente creado correctamente.</Alert>}
          ...
          <p className="text-muted mb-3">
            Complete la información del cliente y haga clic en <b>Crear Cliente</b>.
          </p>
