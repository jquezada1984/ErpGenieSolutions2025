...
const NuevoProveedor: React.FC = () => {
  ...
  const onSubmitRHF = useCallback(async (values: NuevoTerceroFormValues) => {
    ...
    try {
      ...
      const errorMessage = e?.response?.data?.error || e?.message || 'Error al crear el proveedor';
      setErr(errorMessage);
    } finally {
      setLoading(false);
    }
    ...
    <div className="configuracion-tercero">
      <Card>
        <CardBody>
          ...
          {ok && <Alert color="success">Proveedor creado correctamente.</Alert>}
          ...
          <p className="text-muted mb-3">
            Complete la información del proveedor y haga clic en <b>Crear Proveedor</b>.
          </p>
