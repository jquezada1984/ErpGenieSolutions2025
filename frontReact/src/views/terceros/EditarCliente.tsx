...
const EditarCliente: React.FC = () => {
  ...
  const isCliente = tercero && tercero.cliente === true;

  useEffect(() => {
    if (!tercero || !isCliente) return;
    const t = tercero;
    reset({
    ...
    <div className="configuracion-tercero">
      <Card>
        <CardBody>
          <div className="d-flex justify-content-between align-items-center mb-4">
            ...
            <div>
              ...
              <Button
                ...
                disabled={loading || !hasChanges || loadingQuery || !isCliente}
              >
                {loading ? (
                  <>
          ...
          {errorQuery && (
            <Alert color="danger">
              Error al cargar el cliente: {errorQuery.message}
            </Alert>
          )}

          {tercero && !isCliente && (
            <Alert color="warning">
              Este registro no es un cliente. Solo se pueden editar terceros con rol Cliente.
            </Alert>
          )}
          ...
          {loadingQuery && !tercero && (
            <div className="text-center py-4">
              <Spinner />
              <p className="mt-2 text-muted">Cargando cliente...</p>
            </div>
          )}

          {!loadingQuery && !tercero && id && !errorQuery && (
            <Alert color="warning">Cliente no encontrado.</Alert>
          )}
          ...
          <p className="text-muted mb-3">
            Modifique la información del cliente y haga clic en <b>Guardar Cambios</b>.
          </p>
