...
from services import cuenta_impuesto_service as svc
...
@cuenta_impuesto_bp.route('/cuentas-impuestos', methods=['GET', 'OPTIONS'])
def listar():
...
@cuenta_impuesto_bp.route('/cuentas-impuestos', methods=['POST', 'OPTIONS'])
def crear():
    if request.method == 'OPTIONS':
        return '', 204
    id_empresa = _empresa_id()
...
@cuenta_impuesto_bp.route('/cuentas-impuestos/<id_row>', methods=['PUT', 'OPTIONS'])
...
@cuenta_impuesto_bp.route('/cuentas-impuestos/<id_row>', methods=['DELETE', 'OPTIONS'])
def eliminar(id_row):
    if request.method == 'OPTIONS':
        return '', 204
    id_empresa = _empresa_id()
